 ...
 */
public class LoginIntoSite extends ActionSupport implements FormRequirements, SessionAware {
        ...
        try {
            ...
            if (!getSessionStatus()) {
                if (getAdmin() != null) {
                    ...
                    formSubmissionUtils.sendNotificationEmail("admin", emailMessage());
                    setAdminTickets(formSubmissionUtils.getTicketsForAdmin(login.getUsername()));
                    return ADMIN_LOGIN;
                }
                if (getUser() != null) {
                    logger.info("The user has logged in: " + user.toString());
                    sessionMap.put(getClientUserId(), login.getUsername());
                    setUserTickets(formSubmissionUtils.getTicketListForClient(login.getUsername()));
                    return SUCCESS;
                }
            } else {
